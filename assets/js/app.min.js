...
$(function(){
						...
						matchs:[
							{
								time:'11.20  16:00',
								teamA:'dasa',
								teamAimg:'./dist/images/teamA.jpg',
								teamB: 'FIVE',
								teamBimg: './dist/images/teamB.jpg',
								grade: '0:0',
								isOff: true,
								link: '#'
							},							
	...
	$(".game-box").each(function(){
		var oList = $(this).find('.group-list');
		var groupCount = oList.find('li').length;
		var groupWid = 1170 / groupCount;
		oList.find('li').width(groupWid);
		oList.find('.group-hr').css({
			'width': groupWid * (groupCount - 1) + 'px',
			'left': groupWid / 2 + 15 + 'px'
		});
	});	

	$('.game-list>li').eq(0).addClass('active');	
	$('.game-body>.game-box').eq(0).addClass('game-active');
	$('.group-list>li').eq(0).addClass('active');	
	$('.game-active>.group-body>.group-box').eq(0).addClass('group-active');

	$('.game-list>li').each(function(index){
		$(this).on('click',function(){	
			$('.game-list').find('.active').removeClass('active');
			$('.game-list>li').eq(index).addClass('active');		
			$('.game-body').find('.game-active').removeClass('game-active');
			$('.game-body>.game-box').eq(index).addClass('game-active');
			$('.group-body').find('.group-active').removeClass('group-active');
			$('.game-active>.group-list>li').eq(0).addClass('active');
			$('.game-active>.group-body>.group-box').eq(0).addClass('group-active');
		});
	});
	...
	$('.group-list>li').each(function(index){
		$(this).on('click',function(){
			...
			$('.group-list>li').eq(index).addClass('active');	
			$('.group-body').find('.group-active').removeClass('group-active');
			$('.group-body>.group-box').eq(index).addClass('group-active');			
		});
	});

	$('.left-arrow').on('click',function(){
		var matchList = $('.group-active>.match-box>.match-slide>.match-list');
		var str = matchList.css('transform');
		var x = getLeft(str);
		if (x < 0){
			x += 277;
			matchList.css('transform','translateX('+x+'px)');
		}else{
			return false;
		}
	});
	...
	$('.right-arrow').on('click',function(){
		var matchList = $('.group-active>.match-box>.match-slide>.match-list');
		var mCount = matchList.children().length;
		var str = matchList.css('transform');
		var x = getLeft(str);
		if (x > -277 * (mCount - 4)){
			x -= 277;
